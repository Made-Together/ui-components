...
Turborepo monorepo (pnpm workspaces, Node >= 20, see `.nvmrc`: 20.19.5). The `@togetheragency/ui` package is the actual deliverable — a headless React component library. The `apps/playground` (port 3000) and `apps/docs` (port 3001) Next.js 16 apps exist to develop and showcase those components.
...
- Filter to one workspace: `pnpm exec turbo dev --filter=playground` (or `docs`, `@togetheragency/ui`).
- New UI component scaffold: `pnpm --filter @togetheragency/ui generate:component`.

There is no test runner wired up yet — `*.test.tsx` files exist but no `test` script is defined.

## Architecture notes

- **`@togetheragency/ui` exports raw source**: `"exports": { "./*": ["./src/*/index.ts", "./src/*.tsx"] }`. No build step — consumers (Next.js apps) transpile the source directly. Import as `@togetheragency/ui/<name>`: folder-based components resolve to `./src/<name>/index.ts` first, falling back to single-file `./src/<name>.tsx`. Folder layout (preferred for new components) needs an `index.ts` re-export.
...
- **Tailwind v4** is a peer dependency of `@togetheragency/ui` (`tailwindcss ^4.0.0`), used by `apps/playground` and `apps/docs` (via `@tailwindcss/postcss`). Components remain headless by default; when styling is needed inside `@togetheragency/ui`, use Tailwind utility classes, and always keep them overridable via `className` (merged last) so consumers can replace any default.
...
- **TypeScript configs** live in `packages/typescript-config` (`base.json`, `nextjs.json`, `react-library.json`) and are referenced via `@togetheragency/typescript-config` workspace dep.
...
- Avoid introducing styling opinions into @togetheragency/ui.
