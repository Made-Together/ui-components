...
const faqItems = [
  { value: "headless", title: "Is the accordion headless?", content: "…" },
  { value: "a11y", title: "What about keyboard support?", content: "…" },
  { value: "controlled", title: "Can I control the open state?", content: "…" },
];
...
- **`Accordion.Indicator`** is an optional decorative `<span>` that mirrors
  the item's `data-state` — handy for a chevron that rotates on open.
...
The library already applies `-rotate-180` on open by default — you only
need to override or extend if you want a different transform.
    ...
    >
      <Accordion.Heading>
        <Accordion.Trigger className="flex w-full items-center justify-between gap-4 px-4 py-3 text-left text-sm font-medium">
          {item.title}
          <Accordion.Indicator className="grid size-6 place-items-center rounded-full bg-neutral-100">
...
rotates by `-180deg` on open by default.

| Prop        | Type                                     | Default | Description                                                                                  |
| ----------- | ---------------------------------------- | ------- | -------------------------------------------------------------------------------------------- |
| `className` | `string`                                 | —       | Merged via `cn` — defaults include the rotate-on-open transition you can override or extend. |
| `children`  | `React.ReactNode`                        | —       | The icon (chevron, plus/minus, etc.) to render inside the indicator.                         |
